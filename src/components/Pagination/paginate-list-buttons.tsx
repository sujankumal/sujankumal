export default function pageListButtons(currentPage: number, numberOfPages: number, setCurrentPage: Function, totalButtonDisplayOnPagination:number){
    ...
    return list_page.map((val: number, index) => {
        ...
        return (
            <>
                {
                    (shouldDisplayDotDotDotAtEnd || shouldDisplayDotDotDotAtStart)
                        ? <li>
                            <button disabled={true} className="flex hover:bg-white hover:text-gray-500 items-center justify-center px-3 h-8 leading-tight text-gray-500 bg-white border border-gray-300 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">...</button>
                        </li>
                        : null
                }
                {
                    (
                        ...
                        <li key={index}>
                            <button onClick={() => { (currentPage === val) ? null : setCurrentPage(val) }}
                                className={`flex w-8 items-center justify-center px-3 h-8 leading-tight text-gray-500 bg-white border hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white ${(val == currentPage) ? "border-b-teal-600 dark: border-b-teal-600" : ""}`}>
                                {val}
                            </button>
                        </li>
                        : null
