...
const jokes:Array<PostType> = await fetchJokes().then((data)=>{
    console.log("Post Jokes:", data);
    return data;
  });    
            ...
            <div className="mb-8 p-4 md:m-8 md:col-span-3">
                {
                    jokes?.map((post:PostType, index)=>{
                        ...
                        <header className="mt-5 text-center">
                          ...
                          <div className="mb-2">
                            <h2>
                              <Link href={APP_BASE_URL+"jokes/"+post.id} className="text-teal-600">{post.title}</Link>
                            </h2>
                          </div>
                        </header>
